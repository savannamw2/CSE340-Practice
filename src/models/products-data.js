// Sample data for products functionality
const productsData = {
    categories: {
        mens: {
            name: "Men's Clothing",
...
// Get all categories
export const getAllCategories = async () => Promise.resolve(productsData.categories);

// Get category data
export const getCategory = async (categoryId) => {
    // Simulate async database call
    return Promise.resolve(productsData.categories[categoryId] || null);
};
...
export const getRandomProduct = async () => {
    const categories = Object.keys(productsData.categories);
    const randomCategory = categories[Math.floor(Math.random() * categories.length)];
    const category = productsData.categories[randomCategory];
